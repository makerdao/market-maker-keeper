...
from market_maker_keeper.etoro_market_maker_keeper import EtoroMarketMakerKeeper
...
class TestEtoroMarketMakerKeeper:
    @staticmethod
    ...
    @staticmethod
    def synchronize_orders_once(keeper: EtoroMarketMakerKeeper):
        keeper.synchronize_orders()
        keeper.order_book_manager.wait_for_stable_order_book()

    @staticmethod
    def synchronize_orders_twice(keeper: EtoroMarketMakerKeeper):
        for _ in range(0, 2):
            keeper.synchronize_orders()
            keeper.order_book_manager.wait_for_stable_order_book()
    ...
    def test_should_create_orders_on_startup(self, deployment: Deployment, tmpdir):
        ...
        # and
        keeper = EtoroMarketMakerKeeper(args=args(f"--eth-from {deployment.our_address} "
                                                  f"--tub-address {deployment.tub.address} "
                                                  f"--etoro-address {deployment.otc.address} "

...
#
# This new leverj code takes advantage of Reduce Only orders which require 0 captial,
# to reduce your position. Note for starting state the DAI deposited on gluon chain 
# should be much greater than 3*DAI notional amounts used in initial config, if not 
# reduce your config sizes for orders placed
...
class LeverjMarketMakerKeeper:
    ...
    def __init__(self, args: list):
        ...
        parser.add_argument("--buffer-factor", type=float, default=0.6,
                            help="Should not go below 0.5, basically how much capital can you use on one side of quotes")
    ...
    def _allocate_to_pair(self, total_available):
        ...
        if ((base == product['baseSymbol']) and (quote == product['quoteSymbol'])):
            if ((target_price is None) or (target_price.buy_price is None) or (target_price.sell_price is None)):
                ...
                quote_allocation = total_available_wad/(number_of_partitions_for_allocation*buffer_adjustment_factor)
    ...
    def sumMaxSellAmounts(self) -> Wad:
        ...
        sell_bands_list = self.bands_config.get_config(self.spread_feed.get()[0])['sellBands']
        sum_max_sell =0.0
    ...
    def sumMaxBuyAmounts(self) -> Wad:
        #amounts produced are in base token
        buy_bands_list = self.bands_config.get_config(self.spread_feed.get()[0])['buyBands']
        target_price = self.price_feed.get_price()
        sum_max_buy =0.0
        ...
        if ((target_price is None) or (target_price.buy_price is None) or (target_price.sell_price is None)):
            self.logger.info(f'target_price is not available, defaulting to large sum max buy amount')
            return Wad.from_number(2.0) * self.sumMaxSellAmounts()
        else:
            average_price = (target_price.buy_price + target_price.sell_price)/Wad.from_number(2)
            baseTokenSumBuyAmount = Wad.from_number(1.2)*Wad.from_number(sum_max_buy)/average_price
    ...
    def place_orders(self, new_orders: List[NewOrder]):
        def place_order_function(new_order_to_be_placed):
            ...
            if new_order_to_be_placed.is_sell == True and open_position_for_base > maxSellAmountConfig:
                ...
                order_id = str(self.leverj_api.place_order(self.pair(), price, 'LMT', new_order_to_be_placed.is_sell, price, amount, leverage_in_wad, True))
            elif open_position_for_base < Wad.from_number(-1)*maxBuyAmountConfig and new_order_to_be_placed.is_sell == False:

...
from pyexchange.liquid import LiquidApi, Order
...
class LiquidMarketMakerKeeper:
    ...
    def __init__(self, args: list):
        ...
        parser.add_argument("--liquid-secret-key", type=str, required=True,
                            help="Secret key for the liquid API")

        parser.add_argument("--liquid-timeout", type=float, default=9.5,
                            help="Timeout for accessing the liquid API (in seconds, default: 9.5)")
        ...
        self.liquid_api = LiquidApi(api_server=self.arguments.liquid_api_server,
    ...
    def synchronize_orders(self):
        ...
        our_buy_balance = self.our_available_balance(order_book.balances, self.token_buy()) - Bands.total_amount(self.our_buy_orders(order_book.orders))
        our_sell_balance = self.our_available_balance(order_book.balances, self.token_sell()) - Bands.total_amount(self.our_sell_orders(order_book.orders))
    ...
    def place_orders(self, new_orders: List[NewOrder]):
        def place_order_function(new_order_to_be_placed):
            ...
            order_id = self.liquid_api.place_order(self.pair(), new_order_to_be_placed.is_sell, new_order_to_be_placed.price, amount)
...
if __name__ == '__main__':
    LiquidMarketMakerKeeper(sys.argv[1:]).main()
